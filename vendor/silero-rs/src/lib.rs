...
impl VadSession {
    ...
    pub fn new_from_bytes(model_bytes: &[u8], config: VadConfig) -> Result<Self> {
        ...
        // ort rc.12 returns recoverable builder errors whose recovery payload
        // is intentionally not Send + Sync. Convert them to plain messages
        // before crossing anyhow's thread-safe error boundary.
